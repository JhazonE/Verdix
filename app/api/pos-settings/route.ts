...
export async function GET(request: NextRequest) {
  try {
    ...
    const columns = [
      ...
      { name: 'enable_edit_qty_auth', type: 'BOOLEAN DEFAULT FALSE' },
      { name: 'edit_qty_auth_username', type: 'VARCHAR(255) NULL' },
      { name: 'edit_qty_auth_password', type: 'VARCHAR(255) NULL' },
    ...
    const sql = `
      SELECT
        ...
        enable_edit_qty_auth AS enableEditQtyAuth,
        edit_qty_auth_username AS editQtyAuthUsername,
        edit_qty_auth_password AS editQtyAuthPassword,
        ...
        enable_quick_add_customer AS enableQuickAddCustomer
      FROM pos_settings
      LIMIT 1
    `;
  ...
  try {
    ...
    if (existing.length === 0) {
      ...
      const allowedFields: Record<string, string> = {
        ...
        enableEditQtyAuth: 'enable_edit_qty_auth',
        editQtyAuthUsername: 'edit_qty_auth_username',
        editQtyAuthPassword: 'edit_qty_auth_password',
        ...
        enableQuickAddCustomer: 'enable_quick_add_customer'
      };
